...
pub async fn pull(
    ...
    let mut total_pulled_count = 0;

    let caldav_config = config
        .caldav
        .clone()
        .context("CalDAV config is not provided in config.yaml")?
        .pull
        .context("CalDAV pull server config not provided")?;
    let caldav_server = caldav_config.server.clone();
    let caldav_port = caldav_config.port.unwrap_or(993);

    log::debug!("Pulling CalDAV for account {}...", &email);
    ...
    let res = client
        ...
        .text()
        .await
        .context("unable to odecode body")?;

    let folder_name = format!("{out_dir}/{domain}/{email}");
    ...
    let res_file_name = "calendars.xml";
    ...
    fs::write(bin_file_path, res)
        .context("unable to write file")
        .context("unable to save *.xml file")?;

    // .send()?;

    // if response.status().is_success() {
    //     let body = response.text()?;
    //     println!("Calendars list: {}", body);
    // } else {
    //     println!("Failed to get calendars: {}", response.status());
    // }
    ...
    log::info!(
        "Done in {:?}, {} new messages stored ok.",
        start.elapsed(),
        total_pulled_count
    );
